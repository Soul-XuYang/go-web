...
- 🎮 **游戏系统**：集成三款游戏（猜数字、地图寻路、2048），配备用户个人自身的完整的排行榜系统
- 🌐 **翻译服务**：基于可自定义的大模型的多语言翻译(本项目默认使用Kimi K2)
- ☁️ **天气查询**：集成可自定义定位(本项目默认高德地图api) + 腾讯天气API(无需密钥可免费调用)
...
### 第三方服务
- **翻译服务**: 自定义大模型 (OpenAI 风格 API，默认Kimi K2风格)
- **地图服务**: 高德地图 API (自定义IP定位)
- **天气服务**: 腾讯天气 API
...
启动应用后，可以通过以下地址访问 **Swagger API 文档** ，默认端口:3000:

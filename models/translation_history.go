...
type TranslationHistory struct {
	gorm.Model            // 历史的基本参数
	User           *Users `json:"-" gorm:"constraint:OnUpdate:CASCADE,OnDelete:CASCADE;"` // 外键约束与级联
	UserID         uint   `json:"user_id" gorm:"not null;index"`                          // 用户ID
	SourceText     string `json:"source_text" gorm:"type:longtext;not null"`              // 原文
	TranslatedText string `json:"translated_text" gorm:"type:longtext;not null"`          // 译文

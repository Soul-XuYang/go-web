...
// TestParsePattern 是一个测试函数，用于测试 parsePattern 函数的功能
func TestParsePattern(t *testing.T) {
    // 首先测试 parsePattern 函数处理 "/p/:name" 的情况，期望返回 ["p", ":name"]
	ok := reflect.DeepEqual(parsePattern("/p/:name"), []string{"p", ":name"})
    // 然后测试 parsePattern 函数处理 "/p/*" 的情况，期望返回 ["p", "*"]
	ok = ok && reflect.DeepEqual(parsePattern("/p/*"), []string{"p", "*"})
    // 接着测试 parsePattern 函数处理 "/p/*name/*" 的情况，期望返回 ["p", "*name"]
	ok = ok && reflect.DeepEqual(parsePattern("/p/*name/*"), []string{"p", "*name"})
    // 如果以上任何测试失败，则输出错误信息 "test parsePattern failed" 并终止测试
	if !ok {
